RSpec.describe Kc::Auth::Service do
  let!(:private_key)   { OpenSSL::PKey::RSA.generate(1024) }
  let!(:public_key)    { private_key.public_key }
  let!(:key_resolver)  { Kc::Common::PublicKeyResolverStub.new(public_key) }
  let!(:service)       { Kc::Auth::Service.new(key_resolver) }

  describe "#decode_and_verify" do
    def create_token(private_key, expiration_date, algorithm)
      KeyHelper.create_token(private_key, expiration_date, algorithm, { iss: "happy-path-1", host: "http://example.org" })
    end
    ...
    context "when token is in an invalid format" do
      ...
      it "should raise an error :invalid_format" do
        expect do
          service.decode_and_verify(token)
        end.to raise_error(TokenError, "Not enough or too many segments")
      end
    end
    ...
    context "when token is in a valid format" do
      ...
      context "and token is generated by another private key" do
        ...
        it "should raise an error :verification_failed" do
          expect do
            service.decode_and_verify(token)
          end.to raise_error(TokenError, "Signature verification raised")
        end
      end
